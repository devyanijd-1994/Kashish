...
// ── Step 1: Generate & save PDF (only if not exists) ──
...
// ── Helper: regenerate expired GWT token ──
function regenerateToken($base_url, $client_id, $client_secret, $entity_id) {
    $curl = curl_init();
    curl_setopt_array($curl, [
        CURLOPT_URL            => $base_url . "/user/auth/generate_token",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_POST           => true,
        CURLOPT_POSTFIELDS     => json_encode(["entity_id" => $entity_id]),
        CURLOPT_HTTPHEADER     => [
    ...
    file_put_contents("uploads/debug_sign.txt",
        date('Y-m-d H:i:s') . " REGEN TOKEN:\n" . $resp . "\n\n", FILE_APPEND);
    // token is inside 'response' key as per Digio API
    return $res['response']['id'] ?? $res['id'] ?? '';
}
...
if (!empty($existing_doc_id)) {
    // Already uploaded — get document detail with token
    $curl = curl_init();
    curl_setopt_array($curl, [
    ...
    // If entity not found — reset and do fresh upload
    if (!empty($detail['code']) && $detail['code'] === 'ENTITY_NOT_FOUND') {
        $conn->query("UPDATE ekyc SET digio_doc_id='' WHERE id=$record_id");
        $existing_doc_id = '';
    } else {
        $token_data  = $detail['access_token'] ?? $detail['signing_parties'][0]['access_token'] ?? [];
        $token       = is_array($token_data) ? ($token_data['id'] ?? '') : $token_data;
        $entity_id   = is_array($token_data) ? ($token_data['entity_id'] ?? '') : '';
        $valid_till  = is_array($token_data) ? ($token_data['valid_till'] ?? '') : '';

        // Check if token expired — regenerate if needed
        if (!empty($entity_id) && !empty($valid_till)) {
            $expiry = strtotime($valid_till);
            if ($expiry < time()) {
                // Token expired — regenerate
                $token = regenerateToken($base_url, $client_id, $client_secret, $entity_id);
            }
        }

        if (!empty($token)) {
            // gateway/login with GWT token — skips mobile OTP, goes direct to Aadhaar OTP
...
file_put_contents("uploads/debug_sign.txt",
    ...
    if ($http_code === 200 && !empty($res['id'])) {
        ...
        // GWT token — top level preferred (skips mobile OTP via gateway/login)
        $token_data = $res['access_token'] ?? $res['signing_parties'][0]['access_token'] ?? [];
        $token      = is_array($token_data) ? ($token_data['id'] ?? '') : $token_data;

        $auth_url = $res['signing_parties'][0]['authentication_url']
            ?? $res['authentication_url']
            ?? '';

        if (!empty($token)) {
            // GWT token in gateway/login — skips mobile OTP, direct Aadhaar OTP
            $esign_url = "https://app.digio.in/#/gateway/login/"
                . urlencode($res['id']) . "/"
                . urlencode($token) . "/"
                . urlencode($identifier)
...
// ── Skip PDF preview — redirect directly to eSign ──
if (!empty($esign_url)) {
    header("Location: " . $esign_url);
    exit;
}
?>
<!DOCTYPE html>
