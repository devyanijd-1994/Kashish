...
file_put_contents("uploads/debug_sign.txt",
    ...
    if ($http_code === 200 && !empty($res['id'])) {
        ...
        // auth_url — use this FIRST to skip mobile OTP (goes direct to Aadhaar eSign)
        $auth_url = $res['signing_parties'][0]['authentication_url']
            ?? $res['authentication_url']
            ?? '';

        if (!empty($auth_url)) {
            // Direct Aadhaar eSign — no mobile OTP screen
            $esign_url = $auth_url;
        } elseif (!empty($token)) {
            // Fallback: gateway/login (shows mobile OTP screen)
            $esign_url = "https://app.digio.in/#/gateway/login/"
                . urlencode($res['id']) . "/"
                . urlencode($token) . "/"
                . urlencode($identifier)
