  ...
  return (
    <header style={{ background: '#fff', boxShadow: '0 2px 8px rgba(0,0,0,0.1)', position: 'sticky', top: 0, zIndex: 999 }}>
      <div style={{ display: 'flex', justifyContent: 'center', padding: '0.5rem 1rem' }}>
        <Link to="/"><img src="/assets/images/logoo2.png" alt="Kashish Joshi Research" style={{ height: 'clamp(36px, 6vw, 52px)', width: 'auto' }} /></Link>
      </div>
    </header>
  );
}
...
function HeroSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.number?.trim() || form.number.length < 10) {
    ...
    if (!form.segment) {
      alert('Please select a trading segment.');
      return;
    }
    if (!form.investment) {
      alert('Please select your trading capital.');
      return;
    }

    setLoading(true);
    try {
      console.log('Submitting landing form with data:', {
        name: form.name,
        number: form.number,
        email: form.email,
      ...
      const response = await submitLandingLead({
        name: form.name,
        number: form.number,
        email: form.email,
        segment: form.segment,
      ...
      console.log('Landing form submission successful:', response);
        ...
        {`
          ...
          *, *::before, *::after { box-sizing: border-box; }
          ...
          @media (max-width: 768px) {
            .hero-container { flex-direction: column !important; gap: 1.5rem !important; }
            .hero-content   { text-align: center !important; min-width: auto !important; }
            .hero-form      { min-width: auto !important; max-width: 100% !important; flex: none !important; width: 100% !important; }
            .hero-badge     { font-size: 0.65rem !important; padding: 0.5rem 0.8rem !important; text-align: center !important; }
            .hero-buttons   { justify-content: center !important; }
            .stats-bar      { flex-wrap: wrap !important; }
            .stp-grid       { grid-template-columns: 1fr !important; }
            .testimonials-faq-container { flex-direction: column !important; gap: 2rem !important; }
            .testimonials-section, .faq-section { max-width: 100% !important; }
          }
          ...
          @media (max-width: 480px) {
            ...
            .stp-stats      { flex-wrap: wrap !important; }
          }

          @media (min-width: 769px) {
            .testimonials-faq-container { flex-direction: row !important; gap: 4rem !important; }
            .testimonials-section, .faq-section { flex: 1 !important; max-width: 50% !important; }
          }
        `}
      </style>
      {/* Hero */}
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select value={form.segment} onChange={e => setForm(f => ({ ...f, segment: e.target.value }))} required style={{
                      width: '100%',
                      border: '1px solid rgba(30, 58, 138, 0.08)',
                      borderRadius: '6px',
                      padding: '0.40rem 0.70rem',
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select value={form.investment} onChange={e => setForm(f => ({ ...f, investment: e.target.value }))} required style={{
                      width: '100%',
                      border: '1px solid rgba(30, 58, 138, 0.08)',
                      borderRadius: '6px',
                      padding: '0.40rem 0.70rem',
...
function StatsBar() {
  return (
    <div style={{ background: '#031031', padding: '1rem 1rem' }}>
      <div className="stp-stats" style={{ maxWidth: '700px', margin: '0 auto', display: 'flex', justifyContent: 'center', gap: '0', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '12px', overflow: 'hidden' }}>
        {[{ val: '90%+', label: 'Performance Efficiency' }, { val: '5K+', label: 'Clients' }, { val: '24/7', label: 'Support' }].map((s, i) => (
          <div key={i} style={{ flex: 1, padding: 'clamp(0.5rem, 2vw, 0.85rem) clamp(0.5rem, 2vw, 1rem)', textAlign: 'center', borderRight: i < 2 ? '1px solid rgba(255,255,255,0.15)' : 'none', minWidth: 0 }}>
            <p style={{ color: '#fff', fontWeight: 800, fontSize: 'clamp(0.95rem, 3vw, 1.2rem)', margin: 0 }}>{s.val}</p>
            <p style={{ color: '#00A651', fontSize: 'clamp(0.55rem, 1.5vw, 0.65rem)', fontWeight: 600, letterSpacing: '0.08em', margin: 0 }}>{s.label.toUpperCase()}</p>
          </div>
        ))}
      </div>
    </div>
...
function STPPlans() {
  return (
    <section style={{ background: '#fff', padding: 'clamp(2rem, 5vw, 4rem) clamp(1rem, 4vw, 2rem)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <p style={{ fontSize: 'clamp(0.6rem, 1.5vw, 0.68rem)', fontWeight: 700, color: '#888', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.4rem' }}>STRUCTURED TRADING PLANS</p>
        <h2 style={{ fontFamily: 'system-ui, -apple-system, sans-serif', fontSize: 'clamp(1.4rem, 3vw, 2.4rem)', fontWeight: 400, color: '#1a1a2e', marginBottom: '0.5rem' }}>STP Signal Plans</h2>
        <p style={{ fontSize: 'clamp(0.8rem, 2vw, 0.88rem)', color: '#666', maxWidth: '600px', lineHeight: 1.7, marginBottom: '1.75rem' }}>
        ...
        {/* Stats */}
        <div className="stp-stats" style={{ display: 'inline-flex', flexWrap: 'wrap', border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden', marginBottom: '2.5rem', background: '#f7f8fa' }}>
          {[{ val: '5 Plans', label: 'TRADING SEGMENTS' }, { val: 'Full Day', label: 'DAILY COVERAGE' }, { val: '7 Core', label: 'FEATURES INCLUDED' }].map((s, i) => (
            <div key={i} style={{ padding: 'clamp(0.5rem, 2vw, 0.75rem) clamp(0.75rem, 3vw, 1.5rem)', textAlign: 'center', borderRight: i < 2 ? '1px solid #e2e8f0' : 'none' }}>
              <p style={{ fontWeight: 800, color: '#031031', fontSize: 'clamp(0.85rem, 2vw, 1rem)', margin: 0 }}>{s.val}</p>
              <p style={{ fontSize: 'clamp(0.55rem, 1.2vw, 0.6rem)', fontWeight: 600, color: '#00A651', letterSpacing: '0.08em', margin: 0 }}>{s.label}</p>
            </div>
          ))}
        </div>
        {/* Cards */}
        <div className="stp-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 290px), 1fr))', gap: 'clamp(1rem, 3vw, 1.5rem)' }}>
          {STP_PLANS.map((plan, i) => (
            <div key={i} style={{ border: '1px solid #e2e8f0', borderRadius: '16px', padding: 'clamp(1rem, 3vw, 1.5rem)', display: 'flex', flexDirection: 'column', gap: '0.75rem', boxShadow: '0 2px 12px rgba(3,16,49,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 'clamp(0.58rem, 1.5vw, 0.62rem)', fontWeight: 700, border: '1px solid #031031', color: '#031031', borderRadius: '999px', padding: '0.2rem 0.65rem' }}>
                  ★ {plan.badge}
                </span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <h3 style={{ fontSize: 'clamp(0.85rem, 2vw, 0.95rem)', fontWeight: 700, color: '#031031', margin: 0, lineHeight: 1.3 }}>{plan.title}</h3>
                  <p style={{ fontSize: 'clamp(0.65rem, 1.5vw, 0.72rem)', color: '#888', margin: 0 }}>{plan.market}</p>
                </div>
              </div>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                {STP_FEATURES.map((f, j) => (
                  <li key={j} style={{ display: 'flex', gap: '0.4rem', fontSize: 'clamp(0.72rem, 1.8vw, 0.78rem)', color: '#555' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.4rem', marginTop: '0.25rem' }}>
                {['3 Months', '6 Months', '1 Year'].map((dur, j) => (
                  <div key={j} style={{ background: '#f4f6fb', borderRadius: '6px', padding: '0.4rem', textAlign: 'center' }}>
                    <p style={{ fontSize: 'clamp(0.58rem, 1.4vw, 0.62rem)', fontWeight: 600, color: '#031031', margin: 0 }}>{dur}</p>
                    <p style={{ fontSize: 'clamp(0.6rem, 1.5vw, 0.65rem)', color: '#00A651', fontWeight: 700, margin: 0 }}>Contact Us</p>
                  </div>
                ))}
              </div>
              <a href="https://wa.link/iw4ct4" target="_blank" rel="noreferrer" style={{
                ...
                padding: '0.65rem', textAlign: 'center', textDecoration: 'none', fontWeight: 700, fontSize: 'clamp(0.75rem, 2vw, 0.82rem)',
...
//   // Show 3 cards: prev (partial), current (center), next (partial)
//   // (single image mode � getSlideItems not used)
...
//         borderRadius: '50%', animation: 'blobFloat 9s ease-in-out infinite', zIndex: 0
//       }} />
//       <div style={{
//         position: 'absolute', bottom: '-60px', right: '-60px',
//         width: '280px', height: '280px',
//         background: 'radial-gradient(circle, rgba(34,197,94,0.06) 0%, transparent 70%)',
//         borderRadius: '50%', animation: 'blobFloat 7s ease-in-out infinite reverse', zIndex: 0
//       }} />
...
//           }}>
//             <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#22c55e', animation: 'livePulse 1.5s infinite', flexShrink: 0 }} />
//             <span style={{ fontSize: '0.72rem', color: '#16a34a', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase' }}>
//               Live Results
//             </span>
//           </div>
//           <h2 style={{
...
//             Real screenshots � unfiltered, unedited trading profits.
//           </p>
//         </div>
...
//             margin: '0 auto'
//           }}>
//             {clientProfits.slice(0, 3).map((image, i) => (
//               <div 
//                 key={i}
//                 style={{
//                   position: 'relative',
...
//             <button
//               onClick={() => {
//                 // You can add logic here to show more screenshots or open a modal
//                 console.log('View all screenshots');
//               }}
//               style={{
//                 display: 'inline-flex',
//                 alignItems: 'center',
...
//               View All Results ({clientProfits.length})
//             </button>
//           </div>
//         </div>
//       </div>
...
function OurClientsProfit() {
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(160deg, #f8fafc 0%, #e2e8f0 60%, #f1f5f9 100%)",
        minHeight: "100vh",
        display: "flex",
        flexDirection: "column",
        justifyContent: "center",
    ...
    >
      ...
      >
        <h2
          style={{
            fontSize: "clamp(1.8rem,1vw,2.8rem)",
            fontWeight: "900",
            color: "#1e293b",
          }}
        >
      ...
      >
        ...
        >
          {[...clientProfits, ...clientProfits].map((image, i) => (
            <div
              key={i}
              style={{
                flex: "0 0 auto",
...
function LandingFooter() {
  return (
    <footer style={{
      background: '#f8fafc',
      color: '#374151',
      padding: 'clamp(1.5rem, 4vw, 3rem) clamp(1rem, 4vw, 2rem) clamp(1rem, 3vw, 2rem)',
      textAlign: 'center',
      position: 'relative',
      zIndex: 9999,
      marginTop: '0',
