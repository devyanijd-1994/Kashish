...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $paidservices = $_POST['paidservices'];
    ...
    if ($conn->connect_error) {
        ...
        header('Location: https://kashishjoshiresearch.com/get-intra-trading-calls');
        exit;
    }

    // Check for duplicates
    $stmt = $conn->prepare("SELECT id FROM intradaytips WHERE email = ? OR number = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        $stmt->close();
        $conn->close();
        header('Location: https://kashishjoshiresearch.com/get-intraday-trading-calls');
        exit;
    } else {
        // Prepare and bind
        $stmt = $conn->prepare("INSERT INTO intradaytips (name, number, email, segment, location, sutabletime, investment, brokername, interested, date_and_time) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssssss", $name, $number, $email, $segment, $location, $connecttime, $investment, $brokername, $paidservices, $date_and_time);
