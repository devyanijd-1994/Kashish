...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 function getUserLocation() {
    ...
    // Use a timeout so a slow/unreachable API doesn't block the form
    ...
    if ($response === false) {
        // GeoPlugin unreachable — continue without location
        error_log('GeoPlugin fetch failed for IP: ' . $ip);
    ...
    if ($data === null) {
        error_log('GeoPlugin JSON decode failed');
        return ['city' => 'Unknown', 'region' => 'Unknown', 'country' => 'Unknown'];
    }
